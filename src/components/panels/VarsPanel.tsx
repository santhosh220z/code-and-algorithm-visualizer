  ...
  return (
    <div className="h-full overflow-y-auto scrollbar-thin p-3 space-y-3">
      {step?.loops && step.loops.length > 0 && (
        <div>
          <h4 className="text-[10px] font-semibold uppercase tracking-wider text-[var(--color-text-dim)] mb-1.5">
            Loop Progress
          </h4>
          <div className="flex gap-1.5 flex-wrap">
            {step.loops.map((loop, idx) => (
              <div
                key={`${loop.label}-${idx}`}
                className="flex items-center gap-2 px-2.5 py-1 rounded-lg bg-[var(--color-surface-3)] border border-[var(--color-border)]"
              >
          ...
          <tbody>
            {Object.entries(step!.vars!).map(([key, value]) => (
              <tr key={key} className="border-b border-[var(--color-border)] last:border-0">
                ...
                <td className="py-1.5 font-mono text-[11.5px] text-[var(--color-text)] break-all">{formatValue(value)}</td>
              </tr>
            ))}
          </tbody>
        </table>
